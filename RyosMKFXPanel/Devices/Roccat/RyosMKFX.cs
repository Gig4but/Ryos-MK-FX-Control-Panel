...
namespace RyosMKFXPanel.Devices.Roccat {
	class RyosMKFX :Lightning {
		public override int GetWidth() {
			return 23;
		}
		public override int GetKeysCount() {
			return 110;
		}

		public RyosMKFX() {
			this.keysLight = new byte[GetKeysCount()];
			this.keysLightOld = new byte[GetKeysCount()];
			this.keysColor = new byte[GetKeysCount() * 3];
			this.keysColorOld = new byte[GetKeysCount() * 3];
		}



		private RyosTalkFXConnection _connection = new RyosTalkFXConnection();
		public override string GetDeviceUID() {
			return "RoccatRyosMKFX";//_connection.ToString();
		}

		public override bool Connect() {
			if (!connected) {
				if (_connection.Initialize()) {
					return connected = _connection.EnterSdkMode();
				}
				return false;
			}
			return true;
		...
		public override bool Disconnect() {
			if (connected)
				return _connection.ExitSdkMode();
			return true;
		}
		...
		public override bool SendPacket() {
			if (connected) {
				if (OptimizePacket()) {
					DelaySleep();
					_connection.SetMkFxKeyboardState(keysLight, keysColor, 1);
		...
		public override void KeysColorChangeByMatrix(float[][] Matrix) {
			KeysLightAllOn();
			KeysColorReset();
			int kbk = 0;
			for (int row = GetHeight() - 1; row >= 0; row--) {
				for (int i = 0; i < GetWidth(); i++) {
					...
					//in center horizontal / F5, F6, F7, Q, W, E, R, T, Y, U, I, O, P, [, ], Windows, FN, AppendMenu
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.5f) + (red * Matrix[row][i + 1] * 0.5f)),
							((green * Matrix[row][i] * 0.5f) + (green * Matrix[row][i + 1] * 0.5f)),
							((blue * Matrix[row][i] * 0.5f) + (blue * Matrix[row][i + 1] * 0.5f))
						});
						kbk++;
					}
					...
					//more right /  A, S, D, F, G, H, J, K, L, ;, ', leftALT
						...
						|| (row == 0 && i == 3)) {
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.25f) + (red * Matrix[row][i + 1] * 0.75f)),
							((green * Matrix[row][i] * 0.25f) + (green * Matrix[row][i + 1] * 0.75f)),
							((blue * Matrix[row][i] * 0.25f) + (blue * Matrix[row][i + 1] * 0.75f))
						});
						kbk++;
					}
					//more left / Z, X, C, V, B, N, M, ,, ., /, ALTGR
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.8f) + (red * Matrix[row][i + 1] * 0.2f)),
							((green * Matrix[row][i] * 0.8f) + (green * Matrix[row][i + 1] * 0.2f)),
							((blue * Matrix[row][i] * 0.8f) + (blue * Matrix[row][i + 1] * 0.2f))
						});
						kbk++;
					}
					...
					//1.25 left sized / TAB, leftCTRL
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.75f) + (red * Matrix[row][i + 1] * 0.25f)),
							((green * Matrix[row][i] * 0.75f) + (green * Matrix[row][i + 1] * 0.25f)),
							((blue * Matrix[row][i] * 0.75f) + (blue * Matrix[row][i + 1] * 0.25f))
						});
						kbk++;
					}
					...
					//1,75 sized / CapsLock
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.825f) + (red * Matrix[row][i + 1] * 0.125f)),
							((green * Matrix[row][i] * 0.825f) + (green * Matrix[row][i + 1] * 0.125f)),
							((blue * Matrix[row][i] * 0.825f) + (blue * Matrix[row][i + 1] * 0.125f))
						});
						kbk++;
					}
					...
					//2,15 sized left / leftSHIFT
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.45f) + (red * Matrix[row][i + 1] * 0.45f) + (red * Matrix[row][i + 2] * 0.10f)),
							((green * Matrix[row][i] * 0.45f) + (green * Matrix[row][i + 1] * 0.45f) + (green * Matrix[row][i + 2] * 0.10f)),
							((blue * Matrix[row][i] * 0.45f) + (blue * Matrix[row][i + 1] * 0.45f) + (blue * Matrix[row][i + 2] * 0.10f))
						});
						kbk += 2;
						i++;
					}
					...
					//2,15 sized right / ENTER
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.10f) + (red * Matrix[row][i + 1] * 0.45f) + (red * Matrix[row][i + 2] * 0.45f)),
							((green * Matrix[row][i] * 0.10f) + (green * Matrix[row][i + 1] * 0.45f) + (green * Matrix[row][i + 2] * 0.45f)),
							((blue * Matrix[row][i] * 0.10f) + (blue * Matrix[row][i + 1] * 0.45f) + (blue * Matrix[row][i + 2] * 0.45f))
						});
						kbk++;
						i += 2;
					}
					...
					//2,50 sized right / rightSHIFT
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.2f) + (red * Matrix[row][i + 1] * 0.4f) + (red * Matrix[row][i + 2] * 0.4f)),
							((green * Matrix[row][i] * 0.2f) + (green * Matrix[row][i + 1] * 0.4f) + (green * Matrix[row][i + 2] * 0.4f)),
							((blue * Matrix[row][i] * 0.2f) + (blue * Matrix[row][i + 1] * 0.4f) + (blue * Matrix[row][i + 2] * 0.4f))
						});
						kbk++;
						i += 2;
					}
					...
					//6,50 sized / SPACE
						...
						KeyColorChange(kbk, new float[] {
							((red * Matrix[row][i] * 0.1f) + (red * Matrix[row][i + 1] * 0.15f) + (red * Matrix[row][i + 2] * 0.15f) + (red * Matrix[row][i + 3] * 0.15f) + (red * Matrix[row][i + 4] * 0.15f) + (red * Matrix[row][i + 5] * 0.15f) + (red * Matrix[row][i + 6] * 0.15f) + (red * Matrix[row][i + 7] * 0.05f)),
							((green * Matrix[row][i] * 0.1f) + (green * Matrix[row][i + 1] * 0.15f) + (green * Matrix[row][i + 2] * 0.15f) + (green * Matrix[row][i + 3] * 0.15f) + (green * Matrix[row][i + 4] * 0.15f) + (green * Matrix[row][i + 5] * 0.15f) + (green * Matrix[row][i + 6] * 0.15f) + (green * Matrix[row][i + 7] * 0.05f)),
							((blue * Matrix[row][i] * 0.1f) + (blue * Matrix[row][i + 1] * 0.15f) + (blue * Matrix[row][i + 2] * 0.15f) + (blue * Matrix[row][i + 3] * 0.15f) + (blue * Matrix[row][i + 4] * 0.15f) + (blue * Matrix[row][i + 5] * 0.15f) + (blue * Matrix[row][i + 6] * 0.15f) + (blue * Matrix[row][i + 7] * 0.05f))
						});
						kbk++;
						i += 6;
					}
		...
		public override void KeysLightChangeByMatrix(float[][] Matrix) {
			KeysLightAllOn();
			KeysColorUpdate();
			int kbk = 0;
			for (int row = GetHeight() - 1; row >= 0; row--) {
				for (int column = 0; column < GetWidth(); column++) {
					...
					//BACKSPACE
						...
						keysLight[kbk] = (byte)((Matrix[row][column] + Matrix[row][column + 1] >= 0.5f) ? 1 : 0);
						kbk++;
						column++;
					}
					//BACKSLASH
					...
					//ENTER
						...
						keysLight[kbk] = (byte)((Matrix[row][column] + Matrix[row][column + 1] + Matrix[row][column + 2] >= 0.5f) ? 1 : 0);
						kbk++;
						column++;
						column++;
					}
					//LEFTSHIFT
						...
						keysLight[kbk] = (byte)((Matrix[row][column] + Matrix[row][column + 1] >= 0.5f) ? 1 : 0);
						kbk++;
						kbk++;//79 key is a LIE! (not exist in US -_-)
						column++;
					}
					//RIGHTSHIFT
					...
					//SPACE
						...
						keysLight[kbk] = (byte)((Matrix[row][column] + Matrix[row][column + 1] + Matrix[row][column + 2]
					...
					//NUMPLUS
						...
						keysLight[kbk] = (byte)((Matrix[row - 1][column] >= 0.5f) ? 1 : 0);
						kbk++;
					}
					//NUMENTER
						...
						keysLight[kbk] = (byte)((Matrix[row][column] >= 0.5f) ? 1 : 0);
						kbk++;
					}
				}
			}
